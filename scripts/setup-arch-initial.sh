...
section "Arch Linux Initial Setup"
...
if $NEED_SUDO && ! command_exists sudo; then
    ...
    error "  su -c 'pacman -S --noconfirm sudo'"
    exit 1
fi
...
# ca-certificates is usually part of ca-certificates-utils on Arch
if ! pacman -Qi ca-certificates-utils &>/dev/null; then
    PACKAGES_TO_INSTALL+=("ca-certificates-utils")
fi
...
section "Arch Linux Setup Complete"
...
echo "  ✓ curl"
echo "  ✓ xz"
echo "  ✓ ca-certificates-utils"
echo "  ✓ Nix with flakes support"
echo "  ✓ nix-daemon (systemd service)"
echo ""
info "Arch-specific notes:"
echo "  - Nix is also available in the AUR, but we use the official installer"
echo "  - The nix-daemon runs as a systemd service"
echo "  - You can manage the service with: systemctl status nix-daemon"
echo ""
